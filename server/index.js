...
io.on("connection", (socket) => {
  ...
  socket.on("join-game", async ({ nickname, gameId }) => {
    try {
      if (!gameId.match(/^[0-9a-fA-F]{24}$/)) { // here we are checking whether the gameId is a type of mongoDB id or not
        socket.emit("notCorrectGame", "Please enter a valid game ID");
        return;
      }
      let game = await Game.findById(gameId);
  
      if (game.isJoin) {  // if the game is joinable then only we are going to join
